...
export function MobileNav() {
  ...
  return <>
    ...
    {open ? createPortal(<div className="fixed inset-0 z-50 bg-black/60 p-3"><div className="ml-auto flex h-full w-full max-w-sm flex-col bg-[#f2eee5] p-6 text-[#141414]">
      <div className="flex items-center justify-between"><span className="font-bold tracking-[.18em]">GOLD’S GYM</span><button type="button" aria-label="Close navigation" onClick={() => setOpen(false)} className="grid size-11 place-items-center rounded-full border border-black/15"><X size={20} /></button></div>
